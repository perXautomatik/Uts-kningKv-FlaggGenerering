...
from
     dbo.vwAehAerende arende left outer join dbo.vwAehHaendelse handelse
            on arende.recAerendeID = handelse.recAerendeID
                    left outer join dbo.vwAehHaendelseKoppladeHaendelserMailAttachment x
...
select
    ...
    replace(case  when CHARINDEX('-', right(z.strFileName, 8)) = 4 then right(z.strFileName, 4)
