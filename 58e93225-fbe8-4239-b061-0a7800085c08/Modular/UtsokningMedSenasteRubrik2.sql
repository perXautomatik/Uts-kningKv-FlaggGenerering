...
with
    ...
    ,harBeslut as (select distinct dianr, 'beslut' as hasBes from utanOnödigaHandlingar where rub like '%beslut%' AND rikt = N'Utgående' group by dianr),
    harForbud as (select distinct dianr, N'förbud' as hasForb from utanOnödigaHandlingar where rub = N'Beslut om förbud avlopp' AND rikt = N'Utgående' group by dianr)
