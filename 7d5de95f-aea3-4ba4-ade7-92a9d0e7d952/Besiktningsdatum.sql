select Besiktningsdatum
						Beslutsdatum
     , Datum, ToemningsdispensFrOM, Volym, ExterntTjaenstID, Tömningsintervall, Id
     , Tillsynsobjekt, Anläggningskategori, Anläggningstyp, Anteckning, Bedömning,
       Certifieringstyp, Recipient, Inventering, Status, Text, Vatten, kdecy, kdecX
     , [geometry]::Point(kdecy, kdecX, 3015) AS Shape
from (SELECT A.datBesiktningsdatum       As Besiktningsdatum
	  ...
	  INNER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrEaAvloppsanlaeggning AS AA
	  ON A.recAvloppsanlaeggningID = AA.recAvloppsanlaeggningID
	  INNER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrTillsynsobjekt AS T
	  ON AA.recTillsynsobjektID = T.recTillsynsobjektID
	  LEFT OUTER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrEaAnlaeggningstyp AS AT
	  ON A.recAnlaeggningstypID = AT.recAnlaeggningstypID
	  LEFT OUTER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrEaAnlaeggningskategori AS AKI
	  ON A.recAnlaeggningskategoriID = AKI.recAnlaeggningskategoriID
      ...
      union
      SELECT A.datBesiktningsdatum              As Besiktningsdatum
	   , A.datBeslutsdatum                  AS Beslutsdatum
	   , 0                                  as Datum
	  ...
	  INNER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrTillsynsobjekt AS T
	  ON A.recTillsynsobjektID = T.recTillsynsobjektID
	  INNER JOIN EDPVisionRegionGotlandAvlopp.dbo.tbTrTillsynsobjektKoordinat AS TK
