...
;with fasWithShape as
   ...
   ,fasInnomSocken as (SELECT socken SockenX,BETECKNING FAStighet, Shape from  fasWithShape inner join
...
insert into @slamz
select * from OnePerFastighet --this should be one per fastighet, but what about houses?
...
select
     ...
     , typ VAantek
     , Byggnadstyp typ, Byggnadstyp , bygTot,coalesce(AnlaggningsPunkt,flagga).STPointN(1)  flagga
...
--order by included desc, statusx, SpillStat,Bytyp
   ...
   ,correctTypOVa as (select *,case when Byggnadstyp is not null and (isnull(typ,'') <> 'Spillvatten:Antaget(spill)'
		      AND isnull(typ,'') <> 'Avlopp:Antaget(spill)') then 'true' end harbyggtypInteIVa from toTeamVatten)
    ,fs as (Select q.FAStighet, Status from sde_miljo_halsoskydd.gng.FLAGGSKIKTET_P byggnad_yta inner join @fastighetsYtor q on byggnad_yta.Shape.STWithin(q.shape) = 1)
   ,FiltreraMotFlaggskiktet as(
	select
	   case when (statusx = 'röd' OR (statusx is null AND  harbyggtypInteIVa  = 'true' ))
	    then 'x' end IrodUtsokning,
	   case when (fs.fastighet not in(select fastighet from toTeamVatten where statusx = 'röd') and  harbyggtypInteIVa  = 'true' )
		then 'x' end IgronUtsokning
	    ,case when isnull(harbyggtypInteIVa,'nej') = 'true' then 'nej' else 'ja' end bla
	 , statusx, SockenX, ttv.fastighet, Diarienummer, Fastighet_tillstand, Beslut_datum, utförddatum, Anteckning, egetOmhändertangandeInfo, VAantek, typ,
	   Byggnadstyp, bygTot, flagga
from
     correctTypOVa ttv left outer join
         fs on fs.FASTIGHET = ttv.fastighet

where fs.Status is null and flagga is not null

)
select fastighet, concat('röd:',isnull(IrodUtsokning,'inteRöd'),',Va:',bla)
...
from FiltreraMotFlaggskiktet
order by bla,IgronUtsokning,IrodUtsokning,statusx,Byggnadstyp, coalesce(statusx, Diarienummer, Fastighet_tillstand, try_cast(Beslut_datum as varchar), try_cast(utförddatum as varchar),
...
with alias as (select * from toTeamVatten where
