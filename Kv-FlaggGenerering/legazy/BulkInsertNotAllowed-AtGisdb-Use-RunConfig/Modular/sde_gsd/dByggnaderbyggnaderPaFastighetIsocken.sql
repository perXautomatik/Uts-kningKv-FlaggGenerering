IF OBJECT_ID(N'tempdb..#ByggnadPåFastighetISocken') is null OR @rebuiltStatus1 = 1
	...
	END CATCH
	BEGIN TRY
	    DROP TABLE #temp_byggnad_yta
	END TRY BEGIN CATCH
	    select 1
	END CATCH;

	with
	    socknarOfIntresse as (select socken SockenX, concat(Trakt, ' ', Blockenhet) FAStighet, Shape
	...
	from (
	    ...
	    from (Select Byggnadstyp, socknarOfIntresse.fastighet Fastighetsbeteckning, byggnad_yta.SHAPE
		  from (select andamal_1T Byggnadstyp, Shape from sde_gsd.gng.BY_0980) byggnad_yta
		      inner join socknarOfIntresse on byggnad_yta.Shape.STWithin(socknarOfIntresse.shape) = 1) q
	) z
	where orderz = 1
