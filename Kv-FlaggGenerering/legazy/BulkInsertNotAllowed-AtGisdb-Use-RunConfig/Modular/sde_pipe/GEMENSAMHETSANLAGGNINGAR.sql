...
with
  ...
  , sYt                       as (select socken SockenX, concat(Trakt, ' ', Blockenhet) FAStighet, Shape
				  from sde_gsd.gng.AY_0980 x
				      inner join (SELECT value "socken"
						  from STRING_SPLIT(
							  N'Roma,Björke,Dalhem,Halla,Ganthem,Hörsne,Bara,Källunge,Vallstena,Norrlanda,Klinte,Fröjel,Eksta',
  ...
  , q                         as (
    ...
    union all
    select shape, concat('AVTALSABONNENT [Tabell_ObjID: ', OBJECTID, ']')
    from sde_pipe.gng.AVTALSABONNENTER AVTALSABONNENTER
    union all
    select shape, concat('GEMENSAMHETSANLAGGNING: ', GEMENSAMHETSANLAGGNINGAR.GA)
