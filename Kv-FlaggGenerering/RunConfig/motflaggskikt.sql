...
with
	...
	fs as (Select q.FAStighet, Status, flaggor.shape
	       from sde_miljo_halsoskydd.gng.FLAGGSKIKTET_P flaggor
		   inner join (select * from #fastighetsYtor) q
		   on flaggor.Shape.STWithin(q.shape) = 1 or q.fastighet = isnull(flaggor.FASTIGHET, ''))
...
select * into #FiltreraMotFlaggskiktet from FiltreraMotFlaggskiktet qx where qx.status is null
;
