use [tempExcel]
 insert into dbo.resultatRunConf (dnr) values ('fordiga')
--drop table tempdb.dbo.##fordiga
if object_id('tempdb..##fordiga') is null begin
    BEGIN TRANSACTION;
WITH
    SRC1LAGFARa as (SELECT * from  tempdb.dbo.##KALLA where src is not null)
    ,rest as (SELECT fnr from  tempdb.dbo.##KALLA WHERE src is null)
...
, FARDIG
    AS (SELECT FNR, PERSORGNR
		  , format(
		try_cast(
			      IIF(charindex('/', ANDEL) > 0,
				  try_cast(left(ANDEL, charindex('/', ANDEL, 1) - 1) AS FLOAT) /
				  try_cast(right(ANDEL, len(ANDEL) - charindex('/', ANDEL, 1)) AS FLOAT), 0) AS FLOAT), '0.00'
		      )                                                                                                                                                                       ANDELMIN
	     , NAMN
	     , ltrim(replace(replace(ltrim(CONCAT(IIF(FAL_POSTNR IS NULL, FAL_CO,
						      nullif('c/o ' + FAL_CO + ', ', 'c/o , ')), FAL_UTADR1, ' ', FAL_UTADR2, ', ', FAL_POSTNR, ' ', FAL_POSTORT)), '  ', ' '), ' , ', ', ')) ADRESS
	 , FAL_POSTORT                                                                                                                                                                        POSTORT, FAL_POSTNR POSTNR, SRC SOURCE FROM  [3toOneUnion2])
SELECT * into tempdb.dbo.##fordiga from FARDIG
        end ;