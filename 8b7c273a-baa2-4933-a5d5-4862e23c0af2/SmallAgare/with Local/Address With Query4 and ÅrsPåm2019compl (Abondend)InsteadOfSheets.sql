with
    ...
    q as (select ANDEL, POSTORT, POSTNUMMER,
                (select * from string_split(ADRESS,',')) as correctedAdress, (select ress from (select trim(ressWithSpaces) as ress from
                     (SELECT value as ressWithSpaces FROM qvc CROSS APPLY STRING_SPLIT(adress, ',') GROUP BY value) soki) asd
