begin try drop table FromRaw end try begin catch print ERROR_line()end catch;

IF EXISTS(SELECT 1 FROM sys.columns WHERE Name = N'namn' AND Object_ID = Object_ID(N'dbo.Agarlista')) BEGIN
begin try
    with
    ...
    end try begin catch print ERROR_line()end catch END;
begin try drop table FromUnionedNotFiltered end try begin catch print ERROR_line()end catch

begin try
    create table FromUnionedNotFiltered (FNR int, org varchar(30), ANDEL varchar(10), namn nvarchar(200), adress nvarchar(300), POSTORT nvarchar(100), POSTNR nvarchar(100), src varchar(13) not null, badness int, trustScore int not null unique clustered (FNR,org,namn,adress,POSTORT,POSTNR) with (ignore_dup_key = on))
end try begin catch print ERROR_line()end catch; use tempExcel go;
...
end try begin catch print ERROR_line()end catch ;
with
...
insert into correctAndelAndCo select FNR , org , ANDEL, namn , adress ,POSTORT ,  POSTNR, src,BADNESS  from joinWithNoBadness
