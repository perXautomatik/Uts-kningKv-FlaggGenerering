...
with
 FARDIG
    AS (SELECT FNR, org
		  , format(
		try_cast( IIF(slashPos > 0,
				  try_cast(nominator AS float) /
				  try_cast(denominator AS float), 0) AS FLOAT), '0.00'
		      ) ANDEL
	     , NAMN
	     ,  ADRESS
         ,POSTORT, POSTNR, SRC SOURCE,badness FROM @Convertion)

	,checkUneeded as (
		select *,rank() over (partition by fnr order by badness,andel desc ) rankNum from FARDIG
	)
	,leCrem as (select *,row_number() over (partition by fnr order by fnr desc )  rowNum from checkUneeded
       	       WHERE rankNum = 1) --atleast one from each, but no max amount, badness always first, and second to that, largest ägare
...
SELECT * into FromcorrecAndelCo from lecrem
