...
--insert into dbo.resultatRunConf (dnr,antal) values ( 'toInsert',case when object_id('tempdb..##toInsert') is null then 1 else 0 end )
...
with
	...
	SELECT vA.STRDIARIENUMMER Dia,coalesce(nullif(vA.STRFASTIGHETSBETECKNING, ''), va.STRSOEKBEGREPP) kir,STRFNRID fnr,va.strLogKommentar,strAerendeStatusPresent
	    ...
	    INNER JOIN tempdb.dbo.##SOCKENLISTA ON LEFT(coalesce(nullif(va.STRFASTIGHETSBETECKNING, ''), va.STRSOEKBEGREPP), len(SOCKEN)) = SOCKEN
	    LEFT OUTER JOIN
	...
	k2 as (SELECT *  from k)
     ,exluded as (
         select p.DIA,p.KIR,p.FNR,p.strAerendeStatusPresent,p.strLogKommentar from (SELECT K2.*, coalesce(T.DIA, T.FAS) X
		  FROM K2
		      LEFT OUTER JOIN  tempdb.dbo.##EXCLUDED T
		      ON
  ...
  ,toInsert as (select strLogKommentar statuskommentar ,DIA,Fnr from correctFnr)
    ...
    --insert into @inputFnr (id,Diarienummer,Fnr,fastighet,HÄNDELSEDATUM ) --;if object_id(' tempdb..##TRM') is null begin begin  TRANSACTION--SELECT * INTO  tempdb.dbo.##TRM from @INPUTFNR ;--END adressCorrecting = gisTable1 -- don't think the view of gisTable1 has 3 segments, so union is not nessessary.--    ip as (select fnr from @INPUTFNR),
select *, GETDATE() inskdatum
into tempdb.dbo.##toInsert
from toInsert
end
;
...
insert into dbo.resultatRunConf (dnr,antal) values ( 'toInsert',(select count(*) c from tempdb.dbo.##toInsert ))
