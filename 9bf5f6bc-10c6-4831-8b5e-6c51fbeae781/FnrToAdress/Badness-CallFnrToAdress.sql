begin
    ...
    values (1, 'MBNV-2019-3863', 90023816, 'HELLVI VIVLINGS 1:99', getdate())
    ...
    with
	q  as (
	    select (case when namn is null then 1 else 0 end) + (case when POSTNR is null then 1 else 0 end) +
		   (case when postort is null then 1 else 0 end) + (case when adress is null then 1 else 0 end) +
		   (case when PERSORGNR is null then 1 else 0 end) badness
		 , FNR, PERSORGNR, ANDEL, NAMN, INSKDATUM, ADRESS, POSTORT, POSTNR, SOURCE
