...
where recHaendelseID >= 442926
     -- strKommunikationssaett is null and recHaendelseKategoriID = 7 and recDiarieAarsSerieID = 56 and strText like 'mbnv%'

delete from dbo.tbAehHaendelseEnstakaKontakt where 	recHaendelseID in ( select recHaendelseID from @toDelete)
delete from dbo.tbAehHaendelseUser where 		recHaendelseID in ( select recHaendelseID from @toDelete)
delete from dbo.tbAehHaendelseEnstakaFastighet where 	recHaendelseID in ( select recHaendelseID from @toDelete)
delete from dbo.tbAehAerendeHaendelse  where 		recHaendelseID in ( select recHaendelseID from @toDelete)
delete
    from tbAehHaendelse
