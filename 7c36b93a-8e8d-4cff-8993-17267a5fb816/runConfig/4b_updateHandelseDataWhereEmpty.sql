-- makes update as nullvalued entries has already bin asigned as handelser bin created.

declare @dat date = getdate();
declare @StrLogKom varchar(max) = 'Autogenererat ' + cast(@dat as varchar);
...
with    StandardValueAllmanHandling as (select top 1 strHaendelseStatusPresent,strHaendelseStatusLogTyp,strHaendelseStatusLocalizationCode,
             88 c, --recKommunID
             4 as recKontaktRollId from tbAehHaendelseData where strHaendelseStatusLocalizationCode is not null order by intRecnum desc)
   	,inputProperlyFormated       as (select distinct [personnr/Organisationnr] org, Dnr, Postadress, try_cast(POSTNR as nvarchar) POSTNR, POSTORT, Ägare, source, fastighet,recHaendelseID from ##fannyUtskick)
