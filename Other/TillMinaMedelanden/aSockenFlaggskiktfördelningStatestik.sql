...
IF object_id('tempdb..#fnrz') is null
	...
	WITH
	    FILTRERADEFAST AS (SELECT SOCKEN SOCKENX, concat(TRAKT, ' ', BLOCKENHET) FASTIGHET, SHAPE
				   ...
				   INNER JOIN
			       (SELECT VALUE "socken"
				FROM STRING_SPLIT(N'Källunge,Vallstena,Hörsne,Bara,Norrlanda,Stenkyrka',
						  ',')) AS SOCKNAROFINTRESSE
	  ...
	  , selection      as (select OBJECTID
				   ...
				   left OUTER JOIN FILTRERADEFAST
			       ...
			       where coalesce(FILTRERADEFAST.FASTIGHET, Flaggskiktet_p_evw.FASTIGHET,
					      Flaggskiktet_p_evw.FASTIGHET_TILLSTAND) is not null and
				     SOCKENX is not null)

	  , fastigheter    as (select max(selection.SOCKENX)                                 socken
	  ...
	  , fnrz           as (select fnr, beteckning, socken, fastigheter.status
			       from sde_geofir_gotland.gng.FA_FASTIGHET q
				   inner join fastigheter on FASTIGHETER.F = beteckning)
	select *
	into #Fnrz
	from FNRZ;
    end ELSE begin
    drop table #fnrz goto startz
end
go
;
...
if (select 1) is null
	...
	with
	  ...
	  , grouped            as (select FNR
					...
					, FAL_POSTORT
				   from ANDELAD
				   GROUP BY FNR, PERSORGNR, NAMN, FAL_CO, FAL_UTADR1, FAL_UTADR2, FAL_POSTNR
					  , FAL_POSTORT)
	  , medFastighetsAgare as (
	    select FNR
		 ...
		 , ltrim(
		    CONCAT(case when FAL_POSTNR is null then fal_co else nullif('c/o ' + FAL_CO + ', ', 'c/o , ') end,
	    ...
	    select REALESTATEKEY
		 ...
		 , format(case when charindex('/', SHAREPART, 1) > 0 then
				       try_cast(left(SHAREPART, charindex('/', SHAREPART, 1) - 1) as float) / try_cast(
					   right(SHAREPART, len(SHAREPART) - charindex('/', SHAREPART, 1)) as float)
			  end, '0.00') ANDEL
		 , Name                NAMN
		 , regdt
		 , Address
		 ...
		 , null                postnr
		 , 'info_CurrentOwner'
	    from sde_geofir_gotland.gng.info_CurrentOwner --sde_geofir_gotland.gng.FASTIGHETSÄGARE
