...
CREATE TABLE #Fastigheter (FASTIGHET NVARCHAR(250), socken nvarchar(100), status nvarchar(100)
)
INSERT
...
select coalesce(FASTIGHETERQ.SOCKEN, FASTIGHETERSOL.SOCKEN) socken
     , coalesce(FASTIGHET, F)                               Fastighet
     , isnull(FASTIGHETERSOL.STATUS, 'Saknas')              SkiktFlagga
     , isnull(FASTIGHETERQ.STATUS, 'saknas')                ExcelFlagga
from fastigheterSol
    full OUTER join fastigheterq on fastigheterSol.fastighet = fastigheterq.f
where (fastighet is null or f is NULL and coalesce(FASTIGHETERQ.STATUS, FASTIGHETERSOL.STATUS) = 'röd') or
      (FASTIGHETERQ.STATUS != FASTIGHETERSOL.STATUS and f is not null)