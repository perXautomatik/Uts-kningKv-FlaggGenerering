...
begin    ;
WITH
    FILTRERADEFAST AS (SELECT SOCKEN SOCKENX, concat(TRAKT, ' ', BLOCKENHET) FASTIGHET, SHAPE FROM SDE_GSD.GNG.AY_0980 X INNER JOIN
        (SELECT VALUE "socken" FROM STRING_SPLIT(N'Källunge,Vallstena,Hörsne,Bara,Norrlanda,Stenkyrka', ',')) AS SOCKNAROFINTRESSE ON X.TRAKT LIKE SOCKNAROFINTRESSE.SOCKEN + '%')
