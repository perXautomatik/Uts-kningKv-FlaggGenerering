...
(
select socken,count(*) flaggor from
     (Select left(coalesce(s.FASTIGHET,f.fastighet) ,charindex(' ',coalesce(s.FASTIGHET,f.fastighet))) socken
     	,f.fastighet ffast
     	,s.FASTIGHET SFAST
		from
     (select case when left(fastighet, charindex('', fastighet)) not in (
		'ARDRE ', 'Dalhem, ', N'Dalhäm ', N'Fröjels ', 'GAMMELGARNS ', 'GANTHEMS ', 'GERUMS ', 'HALHEM ', 'HALLS ', 'Havdem ', N'Hörne ', N'Käälunge ', 'LUMMELINDA ', 'Norlanda ', 'Stenkyka ', 'Stenstugu ', 'STORA ')
		then
            nullif(fastighet,'') end

                fastighet,shape from
     sde_miljo_halsoskydd.gng.FLAGGSKIKTET_P) f
