...
TableInitiate:
IF OBJECT_ID('tempdb..#SockenYtor') IS NULL goto SockenYtor else
 INSERT INTO @statusTable
        select 'preloading#SockenYtor'
             ,CURRENT_TIMESTAMP,(select count(*) from #SockenYtor)
;
...
IF OBJECT_ID('tempdb..#SockenYtor') IS NULL
    ...
    SELECT BETECKNING FAStighet, fnr Fnr_FDS, x.Shape INTO #SockenYtor
    from (select fa.FNR,fa.BETECKNING , fa.TRAKT, yt.Shape
    	from sde_geofir_gotland.gng.FA_FASTIGHET fa inner join sde_gsd.gng.REGISTERENHET_YTA yt on fa.FNR = yt.FNR_FDS) x
...
IF OBJECT_ID(N'tempdb..#ByggnadPåFastighetISocken') is null OR @rebuiltStatus1 = 1
    ...
    begin BEGIN TRY DROP TABLE #ByggnadPåFastighetISocken END TRY BEGIN CATCH select 1 END CATCH
      ...
      ;
    with
  	byggnaderISocken as (select socken,b.* from sde_gsd.gng.BYGGNAD b inner join @socknarOfInterest so on b.shape.STIntersects(so.Shape) =1)

  	   ,ByggnadPaFastighetISocken as (select row_number() over (partition by sy.FAStighet order by bis.andamal1 ) orderz, sy.FAStighet,sy.Fnr_FDS, bis.* from #SockenYtor sy inner join byggnaderISocken bIS on bis.Shape.STIntersects(sy.Shape) = 1)
...
IF OBJECT_ID(N'tempdb..#egetOmhändertagande') is null  OR @rebuiltStatus1 = 1
 ...
 with
          sYt as (select socken SockenX,beteckning FAStighet, Shape  from (select fa.FNR,fa.BETECKNING , fa.TRAKT, yt.Shape from sde_geofir_gotland.gng.FA_FASTIGHET fa inner join sde_gsd.gng.REGISTERENHET_YTA yt on fa.FNR = yt.FNR_FDS) x inner join
...
IF OBJECT_ID('tempdb..#spillvaten')is null  OR @rebuiltStatus1 = 1
    begin BEGIN TRY DROP TABLE #spillvaten END TRY BEGIN CATCH select 1 END CATCH
     ...
     ;with
        Va_planomraden_171016_evw as   (select shape,dp_i_omr,planprog,planansokn from sde_va.gng.Va_planomraden_171016_evw)
        ,sYt as (select socken SockenX,beteckning FAStighet, Shape  from (select fa.FNR,fa.BETECKNING , fa.TRAKT, yt.Shape from sde_geofir_gotland.gng.FA_FASTIGHET fa inner join sde_gsd.gng.REGISTERENHET_YTA yt on fa.FNR = yt.FNR_FDS) x inner join
        (SELECT value "socken" from STRING_SPLIT(N'Björke,Dalhem,Fröjel,Ganthem,Halla,Klinte,Roma', ',')) as socknarOfIntresse
         ...
         ,q as (
            select shape, concat(typkod,':',status,'(spill)') typ from sde_va.gng.VO_Spillvatten_evw VO_Spillvatten_evw union all
            select shape, concat('AVTALSABONNENT [Tabell_ObjID: ',OBJECTID,']') as c
            from sde_va.gng.AVTALSABONNENTER AVTALSABONNENTER union all
            select shape, concat('GEMENSAMHETSANLAGGNING: ',GEMENSAMHETSANLAGGNINGAR.GA) as c2
            from sde_va.gng.GEMENSAMHETSANLAGGNINGAR GEMENSAMHETSANLAGGNINGAR union all
            select shape,
                   isnull(coalesce(
...
IF OBJECT_ID(N'tempdb..#Röd')  is null   OR @rebuiltStatus2 = 1
    ...
    with
	    ...
	    ,q as (select                         attUtsokaFran.fastighet,
           ...
           ,
        	...
        	,Q2 as ( select  fastighet,Fastighet_tillstand,Byggnadstyp,Beslut_datum,utförddatum,Anteckning,VaPlan,egetOmhändertangandeInfo,slam,flaggnr,flagga,
              (case when fstatus = N'röd'
                  then (case when (vaPlan is null and egetOmhändertangandeInfo is null) then N'röd'
                          ...
                          then 'KomV?' else (case when null is not null
                              then 'gem' else '?' end) end) end) else fstatus end ) Fstatus
             from q )
