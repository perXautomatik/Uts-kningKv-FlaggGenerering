INSERT INTO #statusTable (medelande) select '#initiating'+@name+'' as alias; declare @tid smalldatetime; IF OBJECT_ID('tempdb..'+@name+'') IS not NULL begin if not (exists(select 1 from )+@name+'') drop table ;+@name+'' end
    ...
    IF OBJECT_ID('tempdb..'+@name+'') IS NULL
	    ...
	    Commit Transaction --C2
        	...
        	select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@4] print 'failed to build' 
        end catch
