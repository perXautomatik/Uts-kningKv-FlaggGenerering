...
IF OBJECT_ID(''tempdb..#ByggnadPåFastighetISocken'') IS NULL Begin
    with byggnad_yta as (select andamal_1T Byggnadstyp, Shape from sde_gsd.gng.BY_0980),
        ...
        select Fastighetsbeteckning, Byggnadstyp,shape ByggShape into #ByggnadPåFastighetISocken from (select *, row_number() over (partition by Fastighetsbeteckning order by Byggnadstyp ) orderz from q) z where orderz = 1;END
	')
...
IF OBJECT_ID(''tempdb..#Socken_tillstånd'') IS NULL Begin;
      with
...
IF OBJECT_ID(''''tempdb..#taxekod'''') IS NULL Begin;
