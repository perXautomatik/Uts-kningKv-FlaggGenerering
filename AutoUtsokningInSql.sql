/*Insert Databases names into SQL Temp Table*/begin try
    declare @statusTable table
                         (
                             medelande NVARCHAR(max),
                             start     smalldatetime,
                             rader     integer
                         );
    TableInitiate:/*dropTabels?*/INSERT INTO @statusTable (medelande) select '#initiating#slam';
    ...
    INSERT INTO @statusTable
    select 'preloading#röd', CURRENT_TIMESTAMP, count(*)
    from #rödINSERT INTO @statusTable (medelande)
    ...
    goto TableInitiate /*--koden skall inte exekveras i läsordning, alla initiate test moste först slutföras, sedan går programmet till repportblocket.*/SockenYtor:
    declare @tid smalldatetime set @tid = CURRENT_TIMESTAMP
    INSERT INTO @statusTable (medelande) select 'Starting#SockenYtor'
    BEGIN TRY
        BEGIN TRANSACTION with socknarOfIntresse as (Select N'Björke' "socken"
                                                     Union
                          ...
                          from sde_gsd.gng.AY_0980 x
                                   inner join socknarOfIntresse on left(x.TRAKT, len(socknarOfIntresse.socken)) =
                                                                   socknarOfIntresse.socken Commit Transaction
    end try begin catch
        ROLLBACK TRANSACTION
        insert into @statusTable select ERROR_MESSAGE(), CURRENT_TIMESTAMP, @@ROWCOUNTprint 'failed to build' throw
    end catch
    set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#', @tid, @@ROWCOUNT
    goto TableInitiateByggnadPåFastighetISocken:set @tid = CURRENT_TIMESTAMP
    INSERT INTO @statusTable (medelande) select 'Starting#Byggnadsyta'
    BEGIN TRY
        BEGIN TRANSACTION with byggnad_yta as (select andamal_1T Byggnadstyp, Shape from sde_gsd.gng.BY_0980),
                               q as (Select Byggnadstyp,
                          ...
                          from (select *,
                                       row_number()
                                               over (partition by Fastighetsbeteckning order by Byggnadstyp ) orderz
                                from q) z
                          where orderz = 1;
        Commit Transaction
    end try begin catch
    ...
    Socken_tillstånd:set @tid = CURRENT_TIMESTAMP
    INSERT INTO @statusTable (medelande) select 'Starting#Socken_tillstånd'
    BEGIN TRY
        BEGIN TRANSACTION
    ...
    BEGIN TRY
        BEGIN TRANSACTION
            ...
            select fastighet,
                   concat(nullif(Diarienr + '-', '-'), nullif(Fastighe00 + '-', ' -'),
                          nullif(Fastighet_ + '-', ' -'), nullif(Eget_omhän + '-', ' -'),
                          nullif(Lokalt_omh + '-', ' -'), nullif(Anteckning + '-', ' -'),
            ...
            from (select OBJECTID,
                         ...
                         FAStighet
                  from #SockenYtor sYt
                           left outer join Med_fastighet on fas like '%' + sYt.Fastighet + '%'
                  where fas is not null
                  union all
                  select *
    ...
    spillvaten:
    set @tid = CURRENT_TIMESTAMP INSERT INTO @statusTable (medelande) select 'Starting#spillvaten'
    BEGIN TRY
        BEGIN TRANSACTION with Va_planomraden_171016_evw as (select shape, dp_i_omr, planprog, planansokn
        ...
        BEGIN TRANSACTION
            with slamm as (select strFastBeteckningHel,
               ...
               , slam as (select strFastBeteckningHel,
                                 datStoppdatum =STUFF(
                                         (SELECT distinct ',' + nullif(strDelprodukt + '|', '|') + z2 as nadas
                                          FROM slamm x
                                          where q.strFastBeteckningHel = x.strFastBeteckningHel
                                          FOR XML PATH ('')), 1, 1, '')
                          from slamm q
               ...
               , socknarOfInteresse as (select distinct sockenX socken, fastighet from #SockenYtor)
                byggnader as (select Fastighetsbeteckning fastighet
               , Byggnadstyp
               , ByggShape
                from #ByggnadPåFastighetISocken)
               , vaPlan as (select fastighet
               , typ from #spillvaten)
               ...
               , Anteckning
               , (case
                when not (Beslut_datum
               > DATETIME2FROMPARTS(2003
               , 1
               , 1
               , 1
               , 1
...
select 'tableCreate#Inventering_Bjorke'
BEGIN TRY
    BEGIN TRANSACTION
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_Bjorke'
BEGIN TRY
...
go
BEGIN TRANSACTION
    insert into gng.Inventering_Bjorke (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status, Anteckning,
                                        Utforddatum, Slamhamtning, Antal_byggnader, alltidsant, Shape, skapad_datum,
                                        andrad_datum)
    select (row_number() over (order by fastighet,flaggnr)),
           fastighet,
           Fastighet_tillstand,
           Beslut_datum,
           Fstatus,
           concat(Byggnadstyp, ' ', Anteckning, ' ', nullif(concat('vaPlan: ', VaPlan, ' '), 'vaPlan:  '),
                  nullif(concat('egetOmh: ', egetOmhändertangandeInfo, ' '), 'egetOmh:  ')),
           utförddatum,
           slam,
           flaggnr,
           1,
           ...
           CURRENT_TIMESTAMP
    from #röd
    where Socken = N'Björke';
Commit Transaction


set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_dalhem'
BEGIN TRY
    BEGIN TRANSACTION
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_dalhem'
BEGIN TRY
...
go
BEGIN TRANSACTION
    insert into gng.Inventering_Dalhem (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status, Anteckning,
                                        Utforddatum, Slamhamtning, Antal_byggnader, alltidsant, Shape, skapad_datum,
                                        andrad_datum)
    select (row_number() over (order by fastighet,flaggnr)),
           fastighet,
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_frojel'
BEGIN TRY
    BEGIN TRANSACTION
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_frojel'
BEGIN TRY
...
set @tid = CURRENT_TIMESTAMP - @tid
INSERT INTO @statusTable
select 'rebuilt#', @tid, @@ROWCOUNT
goto TableInitiaterepport:/*--begin try*/select *
                                         from @statusTable
end try
begin
    catchinsert into @statusTable select ERROR_MESSAGE(), CURRENT_TIMESTAMP, @@ROWCOUNTselect * from @statusTable
end catch--/*--Kopierade tabellföreteckning från gng.FLAGGSKIKTET_P*/
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_Bjorke'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_Bjorke
                      (
                          OBJECTID            int not null
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_Bjorke (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                      Anteckning, Utforddatum, Slamhamtning, Antal_byggnader,
                                                      alltidsant, Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         Fastighet_tillstand,
                         Beslut_datum,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = N'Björke'; Commit Transaction set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_dalhem'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_dalhem
                      (
                          OBJECTID            int not null
                              constraint invdalhem_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_Dalhem (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                      Anteckning, Utforddatum, Slamhamtning, Antal_byggnader,
                                                      alltidsant, Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'Dalhem'; Commit Transaction set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_frojel'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_frojel
                      (
                          OBJECTID            int not null
                              constraint invfrojel_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
go
<<<<<<< HEAD
BEGIN TRANSACTION insert into gng.Inventering_frojel (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                      Anteckning, Utforddatum, Slamhamtning, Antal_byggnader,
                                                      alltidsant, Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'fröjel'; COMMIT TRANSACTION set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_ganthem'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_ganthem
                      (
                          OBJECTID            int not null
                              constraint invganthem_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
IF OBJECT_ID(N'tempdb..#egetOmhandertagande') IS NULL
    begin
        ...
        BEGIN TRY
            BEGIN TRANSACTION declare @externalQuery nvarchar(max), @externalparam nvarchar(255), @bjorke nvarchar(255),@dalhem varchar(255),@frojel nvarchar(255),@ganthem varchar(255),@Halla varchar(255),@Klinte varchar(255),@Roma varchar(255)
            ...
            set @externalQuery =
                        ...
                        N',LOKALT_SLAM_P as (select Diarienr,Fastighet_,Fastighe00,Eget_omhän,Lokalt_omh,Anteckning,Beslutsdat,shape from sde_miljo_halsoskydd.gng.MoH_Slam_Lokalt_p_evw), Fas2 as (select *,concat(nullif(LOKALT_SLAM_P.Lokalt_omh + space(1), space(1)), nullif(LOKALT_SLAM_P.Fastighet_ + space(1), space(1)), nullif(LOKALT_SLAM_P.Fastighe00 + space(1), space(1))) fas from sde_miljo_halsoskydd.gng.MoH_Slam_Lokalt_p_evw LOKALT_SLAM_P), Med_fastighet as (select Fastighet_,Fastighets,Eget_omhän,Lokalt_omh,Fastighe00,Beslutsdat,Diarienr,Anteckning,shape LocaltOmH,fas from Fas2 where fas is not null and charindex(char(58), fas) > 0), utan_fastighet as (select Fastighet_,Fastighets,Eget_omhän,Lokalt_omh,Fastighe00,Beslutsdat,Diarienr,Anteckning,utan_fastighet.shape LocaltOmH,FAStighet            fas from (select * from Fas2 where fas is null OR charindex(char(58), fas) = 0) utan_fastighet inner join socknarOfIntresse sYt on sYt.shape.STIntersects(utan_fastighet.Shape) = 1) select Fastighet_, Fastighets, Eget_omhän, Lokalt_omh, Fastighe00, Beslutsdat, Diarienr, Anteckning,LocaltOmH, FAStighet from socknarOfIntresse sYt left outer join Med_fastighet on fas like char(37) + sYt.Fastighet + char(37) where fas is not null union all select * from utan_fastighet ';
            ...
            (
                ...
                egetOmhandertangandeInfo as (concat(nullif('DiaNr: ' + ltrim(Diarienr), 'DiaNr: '),
            ...
            INSERT INTO #egetOmhandertagande (Fastighet_, Fastighets, Eget_omhän, Lokalt_omh, Fastighe00, Beslutsdat,
                                              Diarienr, Anteckning, LocaltOmH, fastighet) exec
                        gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke,
                        @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte,
                        @Roma=@Roma Commit Transaction /*   drop table #egetOmhandertagande*/
        end try begin catch
            ROLLBACK TRANSACTION
...
INSERT INTO #statusTable
select 'preloading#egetOmhandertagande', CURRENT_TIMESTAMP, count(*)
from #egetOmhandertagande;
>>>>>>> bb2cf63... fixat egetomhändertagande
go
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_ganthem'
BEGIN TRY
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_ganthem (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                       Anteckning, Utforddatum, Slamhamtning, Antal_byggnader,
                                                       alltidsant, Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'ganthem'; Commit Transaction set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_halla'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_halla
                      (
                          OBJECTID            int not null
                              constraint invhalla_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_halla'
BEGIN TRY
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_halla (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                     Anteckning, Utforddatum, Slamhamtning, Antal_byggnader, alltidsant,
                                                     Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         Fastighet_tillstand,
                         Beslut_datum,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'halla'; Commit Transaction set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_klinte'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_klinte
                      (
                          OBJECTID            int not null
                              constraint invklinte_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_klinte'
BEGIN TRY
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_klinte (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                      Anteckning, Utforddatum, Slamhamtning, Antal_byggnader,
                                                      alltidsant, Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'klinte'; Commit Transaction set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'tableCreate#Inventering_Roma'
BEGIN TRY
    BEGIN TRANSACTION create table gng.Inventering_Roma
                      (
                          OBJECTID            int not null
                              constraint invRoma_pk primary key,
                          FASTIGHET           nvarchar(150),
                          Fastighet_tillstand nvarchar(150),
                          Arendenummer        nvarchar(50),
                          Beslut_datum        datetime2,
...
set @tid = CURRENT_TIMESTAMP
INSERT INTO @statusTable (medelande)
select 'createIndex#Inventering_Roma'
BEGIN TRY
...
Commit Transaction
go
BEGIN TRANSACTION insert into gng.Inventering_Roma (OBJECTID, FASTIGHET, Fastighet_tillstand, Beslut_datum, Status,
                                                    Anteckning, Utforddatum, Slamhamtning, Antal_byggnader, alltidsant,
                                                    Shape, skapad_datum, andrad_datum)
                  select (row_number() over (order by fastighet,flaggnr)),
                         fastighet,
                         ...
                         CURRENT_TIMESTAMP
                  from #röd
                  where Socken = 'Roma'; Commit Transaction
