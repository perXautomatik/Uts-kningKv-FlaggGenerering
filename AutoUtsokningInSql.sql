...
if (select 1) IS NULL BEGIN TRY
    ...
    Drop table #rodx
    Commit Transaction
 end try begin catch ROLLBACK TRANSACTION  insert into #statusTable select ERROR_MESSAGE() "m",CURRENT_TIMESTAMP "t",@@ROWCOUNT  "c"print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO #statusTable select 'built#',@tid,@@ROWCOUNT ; --koden skall inte exekveras i läsordning, alla initiate test moste först slutföras, sedan går programmet till repportblocket.

go
INSERT INTO #statusTable (medelande) select '#initiating#SockenYtor' as alias;declare @tid smalldatetime; IF OBJECT_ID('tempdb..#SockenYtor') IS not NULL begin if not (exists(select 1 from #SockenYtor)) begin drop table #SockenYtor;end end IF OBJECT_ID('tempdb..#SockenYtor') IS NULL begin try set @tid = CURRENT_TIMESTAMP ;INSERT INTO #statusTable (medelande) select 'Starting#SockenYtor' as alias
BEGIN TRANSACTION
    ...
    INSERT INTO #SockenYtor exec gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma

Commit Transaction
    end try begin catch ROLLBACK TRANSACTION  insert into #statusTable select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@4] print 'failed to build' end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO #statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@3] INSERT INTO #statusTable select 'preloading#SockenYtor',CURRENT_TIMESTAMP,count(*) from #SockenYtor;

go
declare @tid smalldatetime;INSERT INTO #statusTable (medelande) select '#initiating#Byggnader' "a" ; IF OBJECT_ID(N'tempdb..#ByggnadPaFastighetISocken') is not null begin if not (exists(select 1 from #ByggnadPaFastighetISocken)) begin drop table #ByggnadPaFastighetISocken;end end IF OBJECT_ID(N'tempdb..#ByggnadPaFastighetISocken') is null begin set @tid = CURRENT_TIMESTAMP INSERT INTO #statusTable (medelande)  select 'Starting#Byggnadsyta' "a" BEGIN TRY
    BEGIN TRANSACTION
    declare @externalQuery nvarchar(max), @externalparam nvarchar(255), @bjorke nvarchar(255),@dalhem varchar(255),@frojel nvarchar(255),@ganthem varchar(255),@Halla varchar(255),@Klinte varchar(255),@Roma varchar(255)
    set @bjorke=N'björke'set @dalhem = 'Dalhem'set @frojel = N'Fröjel'set @ganthem = 'Ganthem'set @Halla = 'Halla'set @Klinte = 'Klinte'set @Roma = 'Roma';
    set @externalparam = N'@bjorke nvarchar(255) , @dalhem varchar(255) , @frojel nvarchar(255) , @ganthem varchar(255) , @Halla varchar(255) , @Klinte varchar(255) ,  @Roma varchar(255)'
    ...
    select @externalQuery CREATE TABLE #ByggnadPaFastighetISocken (Fastighetsbeteckning nvarchar (250), Byggnadstyp nvarchar(100), ByggShape geometry)

    INSERT INTO #ByggnadPaFastighetISocken exec gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma

    Commit Transaction

   end try begin catch ROLLBACK TRANSACTION  insert into #statusTable select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@5] print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO #statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@6]end  INSERT INTO #statusTable select 'preloading#ByggnadPaFastighetISocken', CURRENT_TIMESTAMP,     count(*) from #ByggnadPaFastighetISocken;

go
    declare @tid smalldatetime;INSERT INTO #statusTable (medelande) select N'#initiating#sockenTillstånd' "a" ; IF OBJECT_ID(N'tempdb..#Socken_tillstand') IS not  NULL begin if not  (exists(select 1 from #Socken_tillstand)) begin drop table #Socken_tillstand;end end IF OBJECT_ID(N'tempdb..#Socken_tillstand') IS NULL begin set @tid = CURRENT_TIMESTAMP INSERT INTO #statusTable (medelande)  select 'Starting#Socken_tillstand' "a" BEGIN TRY BEGIN TRANSACTION
    ...
    INSERT INTO #Socken_tillstand (FAStighet, Diarienummer, Fastighet_tillstand, Beslut_datum, utförddatum, Anteckning, AnlaggningsPunkt) exec gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma

    Commit Transaction
    end try begin catch ROLLBACK TRANSACTION  insert into #statusTable select ERROR_MESSAGE() "M" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@7]print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO #statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@8]end INSERT INTO #statusTable select 'preloading#Socken_tillstand',CURRENT_TIMESTAMP,count(*) from #Socken_tillstand;

go
    declare @tid smalldatetime;INSERT INTO #statusTable (medelande) select N'#initiating#egetOmhandertagande' "a" ; IF OBJECT_ID(N'tempdb..#egetOmhandertagande') IS not NULL begin if not  (exists(select 1 from #egetOmhandertagande)) begin drop table #egetOmhandertagande;end end IF OBJECT_ID(N'tempdb..#egetOmhandertagande') IS NULL begin set @tid = CURRENT_TIMESTAMP INSERT INTO #statusTable (medelande)  select 'Starting#egetOmhandertagande' "a" BEGIN TRY
    BEGIN TRANSACTION
    ...
    INSERT INTO #egetOmhandertagande (Fastighet_,Fastighets,Eget_omhän,Lokalt_omh,Fastighe00,Beslutsdat,Diarienr,Anteckning,LocaltOmH, fastighet) exec gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma
...
go
    ...
    INSERT INTO #Spillvatten exec gisdb01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma
...
go
   ...
   set @externalQuery =
    ...
    INSERT INTO #slamx exec admsql01.master.dbo.sp_executesql @externalQuery, @externalparam, @bjorke=@bjorke, @dalhem=@dalhem, @frojel=@frojel, @ganthem= @ganthem, @Halla=@Halla, @Klinte= @Klinte, @Roma=@Roma,@depoX = @depoX ,@cont = @cont,@grund = @grund,@overtra = @overtra, @hush = @hush, @avctr=@avctr, @budsm = @budsm, @hyra = @hyra
        ;with
         ...
         select *into #slam from (select strFastBeteckningHel, datStoppdatum =STUFF((SELECT char(13) + nullif(' ' + nullif(strDelprodukt + '|', '|') + stuffing, ' ') as n FROM stuffedTypText x where q.strFastBeteckningHel = x.strFastBeteckningHel FOR XML PATH (''), root('MyString'), type).value('/MyString[1]','varchar(max)'), 1, 2, '')from stuffedTypText q group by strFastBeteckningHel) stuffedWithStopDat;
    Commit Transaction
...
go
	declare @tid smalldatetime; begin try set @tid = CURRENT_TIMESTAMP INSERT INTO #statusTable (medelande)  select N'Starting#rodx' "a"; BEGIN TRANSACTION IF OBJECT_ID(N'tempdb..#rodx') IS not NULL begin if (exists(select 1 from #rodx)) begin drop table #rodx INSERT INTO #statusTable (medelande)  select N'purged#rodx' "a";end end
        Commit Transaction
        ;with
