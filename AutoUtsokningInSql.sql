...
if (select null) IS NULL BEGIN TRY
...
goto repport
    ...
    insert into @statusTable select ERROR_MESSAGE() "m",CURRENT_TIMESTAMP "t",@@ROWCOUNT  "c"print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'built#',@tid,@@ROWCOUNT
...
if (select 1) IS NULL BEGIN TRY
    ...
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "m",CURRENT_TIMESTAMP "t",@@ROWCOUNT  "c"print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'built#',@tid,@@ROWCOUNT goto TableInitiate --koden skall inte exekveras i läsordning, alla initiate test moste först slutföras, sedan går programmet till repportblocket.

SockenYtor:
set @tid = CURRENT_TIMESTAMP INSERT INTO @statusTable (medelande)  select 'Starting#SockenYtor' "a"  BEGIN TRY
    BEGIN TRANSACTION
set @query=
...
exec sp_executesql @query;
    ...
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@4] print 'failed to build' end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@3] goto TableInitiate
...
BEGIN TRY
  ...
  set @query= N'select * into #ByggnadPaFastighetISocken from openquery(gisdb01
   ...
   ,byggnad_yta as (select andamal_1T Byggnadstyp, Shape from sde_gsd.gng.BY_0980),
    ...
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@5]
print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@6] goto TableInitiate
...
print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#' "a" ,@tid, @@ROWCOUNT as [@8] goto TableInitiate
...
BEGIN TRY
 ...
 set @query= N'select * into #egetOmhandertagande from openquery(gisdb01
    ...
    Commit Transaction end try begin catch
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "E" , CURRENT_TIMESTAMP "C" , @@ROWCOUNT as [@9] print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#' "a" , @tid,  @@ROWCOUNT as [@10] goto TableInitiate
...
BEGIN TRY
    ...
    Commit Transaction end try begin catch
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "m",CURRENT_TIMESTAMP "t",@@ROWCOUNT  "c" print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#' "a",@tid,@@ROWCOUNT  as [@10] goto TableInitiate
;taxekod:

IF OBJECT_ID(N'tempdb..#Taxekod')  is null
    BEGIN TRY
        BEGIN TRANSACTION
        BEGIN TRY DROP TABLE #Taxekod END TRY BEGIN CATCH select 1 END CATCH
...
begin catch
    ROLLBACK TRANSACTION  insert into @statusTable select ERROR_MESSAGE() "m",CURRENT_TIMESTAMP "t",@@ROWCOUNT  "c"print 'failed to build'  end catch set @tid = CURRENT_TIMESTAMP - @tid INSERT INTO @statusTable select 'rebuilt#',@tid,@@ROWCOUNT goto TableInitiate
