...
with fastighetsfilter    as (Select 'björke' "socken"
    ...
    -- create view fastighetsytor of interest
     ...
     socknarOfIntresse   as (SELECT fastighetsFilter.socken SockenX, concat(Trakt, SPACE(1), Blockenhet) FAStighet, Shape from sde_gsd.gng.AY_0980 x
         inner join fastighetsFilter on left(x.TRAKT, len(fastighetsFilter.socken)) = fastighetsFilter.socken),

    -- create view byggnader innom socken.

     byggnad_yta         as (select andamal_1T Byggnadstyp, Shape from sde_gsd.gng.BY_0980),
     byggnaderInomSocken as (Select Byggnadstyp, socknarOfIntresse.fastighet Fastighetsbeteckning, byggnad_yta.SHAPE
           from byggnad_yta
