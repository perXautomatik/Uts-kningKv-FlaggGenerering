...
EDPVisionRegionGotland.dbo.vwAehAerende z where recHaendelseID = 446413
    select * from
...
insert into EDPVisionRegionGotland.dbo.vwAehHaendelse(
    ...
    recKontaktRollID, recLastHaendelseStatusLogID, strAerendeLocalizationCode, strAerendemening,
    strAerendeStatusPresent, strAerendeTyp, strAerendetypKod, strAvdelningKod, strAvdelningNamn, strEnhetKod,
    strEnhetNamn, strFoervaltningKod, strFoervaltningNamn, strHaendelseKategori, strHaendelseKategoriKod,
    strHaendelseStatusLocalizationCode, strHaendelseStatusLogTyp, strHaendelseStatusPresent, strKommunNamn,
    strPublicering, strRiktning, strRoll, strRubrik, strSignature, datHaendelseDatum,
    , recDiarieSerieID -- år+Diarieserie
    , intSerieStartVaerde -- not used in db? always 1
    --StringDebugg
    , strSoekbegrepp,
    --fixWithJoin
    strAerendeFastighet, strFastighetsbeteckning, recFastighetID, strFnrID,
    recAerendeID, intDiarienummerLoepNummer, strDiarienummer, strDiarieserieAerende, intDiarieAar,
    strGatuadress, strVisasSom, strPostnummer, strPostort,
    --ToWriteMethodsFor
    strHaendelseIdentifiering, recEnstakaKontaktID, recHaendelseID, intLoepnummerHaendelse
    , intRecnum -- intRecnum=recHaendelseID
    , recHaendelseEnstakaKontaktID) from
select
    --copy from refObject
